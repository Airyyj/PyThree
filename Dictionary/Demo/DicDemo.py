...
def on_click(top,base_text,growth_text,time_text):
    ...
    if base_text.get().strip() == '':
        messagebox.showinfo(title='基数', message='基数为空，请输入有效的数值！')
    else:
        base_Num = float(base_text.get())

    if growth_text.get().strip() == '':
        messagebox.showinfo(title='增长率', message='增长率为空，请输入有效的数值！')
    else:
        growth_Rate = float(growth_text.get())

    if time_text.get().strip() == '':
        messagebox.showinfo(title='时间', message='时间为空，请输入有效的数值！')
    elif float(time_text.get()) < 1:
        messagebox.showinfo(title='时间', message='时间小于1，请输入大于或等于1的数值！')
    else:
